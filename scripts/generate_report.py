...
SYSTEM_PROMPT = (
    "你是厭食症（Anorexia Nervosa）領域的資深研究員與科學傳播者。你的任務是：\n"
...
def analyze_papers(api_key: str, papers_data: dict) -> dict:
    ...
    prompt = f"""以下是 {date_str} 從 PubMed 抓取的最新厭食症（Anorexia Nervosa）文獻（共 {paper_count} 篇）。
...
每篇 paper 的 tags 請從以下選擇：飲食障礙治療、FBT家族治療、CBT-E、營養復健與再餵食、神經影像與大腦、內分泌與骨質、心臟與心血管併發症、胃腸道與微生物、運動醫學與RED-S、強迫運動、身體意象、完美主義與認知、情緒調節與創傷、社會媒體與文化、兒少精神醫學、家庭照顧者、流行病學與死亡率、藥物治療、心理治療機制、嚴重頑固型厭食症、跨文化與性別、台灣與東亞。
        ...
        for attempt in range(3):
            try:
                ...
                if result is not None:
                    ...
                    print(
                        f"[WARN] JSON repair failed for {model} attempt {attempt + 1}",
                        file=sys.stderr,
                    )
                    if attempt < 2:
                        time.sleep(5)
                ...
                print(
                    f"[WARN] {model} timed out on attempt {attempt + 1}",
                    file=sys.stderr,
                )
                if attempt < 2:
                    time.sleep(5)
    ...
    for pick in top_picks:
        ...
        <div class="news-card featured">
          <div class="card-header">
            ...
            <span class="{utility_class}">{util}\u5b9a\u7528\u6027</span>
          </div>
...
<title>Anorexia Nervosa Daily &middot; \u53ad\u98df\u75c7\u6587\u737b\u65e5\u5831 &middot; {date_display}</title>
<meta name="description" content="{date_display} \u53ad\u98df\u75c7\u6587\u737b\u65e5\u5831\uff0c\u7531 AI \u81ea\u52d5\u5f59\u6574 PubMed \u6700\u65b0\u8ad6\u6587"/>
<style>
  ...
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .links-banner .banner-link {{ display: flex; align-items: center; gap: 14px; padding: 18px 24px; background: var(--card-bg); border: 1px solid var(--line); border-radius: 24px; text-decoration: none; color: var(--text); transition: all 0.2s; box-shadow: 0 8px 30px rgba(61,36,15,0.04); margin-bottom: 12px; }}
  .links-banner .banner-link:hover {{ border-color: var(--accent); transform: translateY(-2px); box-shadow: 0 12px 40px rgba(61,36,15,0.08); }}
...
<div class="container">
  <header>
    <div class="logo">\U0001f9a0</div>
    <div class="header-text">
      <h1>Anorexia Nervosa Daily &middot; \u53ad\u98df\u75c7\u6587\u737b\u65e5\u5831</h1>
  ...
  <footer>
    ...
    <span><a href="https://github.com/u8901006/anorexia-nervosa">GitHub</a></span>
  </footer>
</div>
</body>
</html>"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate anorexia nervosa daily report HTML"
    )
    ...
    if not papers_data or not papers_data.get("papers"):
        ...
        analysis = {
            ...
            "market_summary": "\u4eca\u65e5 PubMed \u66ab\u7121\u65b0\u7684\u53ad\u98df\u75c7\u6587\u737b\u66f4\u65b0\u3002\u8acb\u660e\u5929\u518d\u67e5\u770b\u3002",
            "top_picks": [],
            "all_papers": [],
            "keywords": [],
            "topic_distribution": {},
