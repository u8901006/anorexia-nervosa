#!/usr/bin/env python3
"""Generate index.html listing all anorexia nervosa daily reports."""

import glob
import os
from datetime import datetime

html_files = sorted(glob.glob("docs/anorexia-*.html"), reverse=True)
links = ""
for f in html_files[:60]:
    name = os.path.basename(f)
    date = name.replace("anorexia-", "").replace(".html", "")
    try:
...
<title>Anorexia Nervosa Daily · 厭食症文獻日報</title>
<style>
...
<div class="container">
  ...
  <h1>Anorexia Nervosa Daily</h1>
  <p class="subtitle">厭食症文獻日報 · 每日自動更新</p>
  <p class="count">共 {total} 期日報</p>
  <ul>{links}</ul>
  <footer>
    <p>Powered by PubMed + NVIDIA Nemotron · <a href="https://github.com/u8901006/anorexia-nervosa">GitHub</a></p>
  </footer>
</div>
</body>
</html>"""
